...
**The Pitfall:**
We initially had `staleTime: 1000 * 60 * 5` (5 minutes). If a user opened Chat A, left, and came back 2 minutes later, React Query would serve the cached messages and *not* refetch. If the other user sent a message during those 2 minutes, it wouldn't show up.
...
To receive messages from the *other* user, we establish WebSocket listeners doing two things:
...
This triggered a background `GET /messages` request. However, because of aggressive HTTP caching layers (like Next.js `fetch` cache) or simple database replication propagation times, that `GET` request returned a slightly **stale** list of messages that *did not yet include the new message*. 
